...
func (s *PaieService) GenerateBulletin(enseignantID, etablissementID, anneeScolaireID uuid.UUID, mois, annee int) (*GenerateBulletinResult, error) {
	...
	heuresPointees := 0.0
	// Calcul simplifié : on compte le nombre de sessions pointées × durée théorique
	// (le calcul exact entrée→sortie sera affiné quand on aura les vraies durées)
	for range sessionsPointees {
		heuresPointees += 2 // TODO: calculer la vraie durée depuis heure_debut/heure_fin de la session
	}
