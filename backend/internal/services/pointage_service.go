...
// Délègue maintenant à l'emploi du temps réel (CreneauEmploiTemps). Si aucun
// créneau n'existe pour ce jour, aucune session n'est créée (plus de créneaux
// fixes arbitraires).
func (s *PointageService) GenerateSessionsForDate(etablissementID uuid.UUID, date time.Time) error {
        edtSvc := NewEmploiTempsService()
        _, err := edtSvc.GenerateSessionsFromDate(etablissementID, date)
        return err
}
