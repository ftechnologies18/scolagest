...
func (h *AnneeScolaireHandler) PromoteStudents(c *gin.Context) {
        var body struct {
                ...
                Decisions       []services.EleveDecision        `json:"decisions"`
        }
        if err := c.ShouldBindJSON(&body); err != nil {
                c.JSON(http.StatusBadRequest, gin.H{"error": "ancienne_annee_id et nouvelle_annee_id requis"})
                return
        }
        result, err := h.svc.PromoteStudents(body.AncienneAnneeID, body.NouvelleAnneeID, body.Decisions)
        if err != nil {
                c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
                return
        }
...
// PreviewPromotion gère POST /api/annees-scolaires/preview
func (h *AnneeScolaireHandler) PreviewPromotion(c *gin.Context) {
        var body struct {
                AncienneAnneeID uuid.UUID `json:"ancienne_annee_id"`
        }
        if err := c.ShouldBindJSON(&body); err != nil {
                c.JSON(http.StatusBadRequest, gin.H{"error": "ancienne_annee_id requis"})
                return
        }
        preview, err := h.svc.PreviewPromotion(body.AncienneAnneeID)
        if err != nil {
                c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
                return
        }
...
func (h *AnneeScolaireHandler) RegisterRoutes(rg *gin.RouterGroup, authMW gin.HandlerFunc) {
        annees := rg.Group("/annees-scolaires", authMW)
        {
                ...
                annees.POST("/preview", h.PreviewPromotion)
