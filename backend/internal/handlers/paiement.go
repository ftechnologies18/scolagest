...
func (h *PaiementHandler) RegisterRoutes(rg *gin.RouterGroup, authMW gin.HandlerFunc) {
        // Caisse (encaissement, reçus) : réservée au CAISSIER et au COMPTABLE.
        // La direction n'y accède pas (séparation des responsabilités : la
        // direction pilote, elle n'encaisse pas au guichet).
        paiements := rg.Group("/paiements", authMW, middleware.RequireRole(
                models.RoleCaissier,
        ...
        // Route imbriquée sous eleves : la lecture des paiements d'un élève reste
        // accessible à tout le personnel authentifié (consultation, pas
        // d'encaissement).
